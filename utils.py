...
def export_to_pdf(calculator: FinancialCalculator, scenarios: Dict, advanced_metrics: Dict) -> io.BytesIO:
    ...
    years = list(range(1, calculator.holding_period + 1))
    cumulative_data = {
    ...
    for name, values in cumulative_data.items():
        ax.plot(years, values, marker='o', label=name)
    ax.axhline(calculator.get_total_initial_investment(), color='orange', linestyle='--', label='Break-even')
    ax.set_xlabel('Year')
    ax.set_ylabel('Cumulative Cash Flow (SAR)')
    ax.legend()
    cash_buf = io.BytesIO()
    fig.tight_layout()
    fig.savefig(cash_buf, format='png')
    plt.close(fig)
    cash_buf.seek(0)
    story.append(Image(cash_buf, width=5*inch, height=3*inch))
    story.append(Spacer(1, 12))

    # Investment breakdown pie chart
    fig, ax = plt.subplots(figsize=(4, 3))
    labels = ['Down Payment', 'Enhancement Costs', 'Loan Amount']
    values = [calculator.down_payment, calculator.enhancement_costs, calculator.get_loan_amount()]
    colors_pie = ['#4285F4', '#34A853', '#FBBC04']
    wedges, texts, autotexts = ax.pie(values, labels=labels, colors=colors_pie, autopct='%1.1f%%', startangle=90)
    ...
    plt.close(fig)
    pie_buf.seek(0)
    story.append(Image(pie_buf, width=4*inch, height=3*inch))
    story.append(Spacer(1, 12))
    ...
    plt.close(fig)
    bar_buf.seek(0)
    story.append(Image(bar_buf, width=4*inch, height=3*inch))
    story.append(Spacer(1, 20))
    ...
    def add_page_number(canvas_obj, doc_obj):
        ...
        canvas_obj.drawCentredString(A4[0] / 2, 0.5 * inch, f"Page {page_num}")
